...
export default async function PriestDashboard() {
  ...
  return (
    <div>
      ...
      <h2 className="mt-8 font-heading text-xl text-maroon-800">
        Upcoming ceremonies
      </h2>
      {upcoming.length === 0 ? (
      ...
      {past.length > 0 && (
        <>
          <h2 className="mt-8 font-heading text-xl text-maroon-800">History</h2>
          <div className="mt-3 space-y-2">
...
function BookingRow({ b, muted }: { b: BookingLite; muted?: boolean }) {
  return (
    ...
    >
      ...
      <span className="text-foreground/60">{b.time_slot}</span>
      ...
      {b.language && (
        <span className="text-xs text-foreground/45">· {b.language}</span>
      )}
      <span
