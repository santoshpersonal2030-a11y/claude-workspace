  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error ?? t("cf.errGeneric"));
      }
      setSent(true);
    } catch (err) {
      setError(err instanceof Error ? err.message : t("cf.errGeneric"));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <input
          type="email"
          aria-label={t("cf.emailLabel")}
          placeholder={t("cf.emailPlaceholder")}
          value={form.email}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <input
          type="tel"
          aria-label={t("cf.phoneLabel")}
          placeholder={t("cf.phonePlaceholder")}
          value={form.phone}
          onChange={update("phone")}
          className={inputClass}
        />
        <input
          aria-label={t("cf.subject")}
          placeholder={t("cf.subject")}
          value={form.subject}
      ...
      <textarea
        aria-label={t("cf.messageLabel")}
        required
        placeholder={t("cf.messagePlaceholder")}
        rows={5}
        value={form.message}
